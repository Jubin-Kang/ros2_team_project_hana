...
# --- UDP Multicast Configuration (same as original script) ---
MCAST_GRP = '224.1.1.1'
MCAST_PORT = 5000
BUFFER_SIZE = 65536
...
def find_optimal_params():
    """
    Receives frames and tests different bilateral filter parameters
    to find the set that minimizes pose jitter on the raw grayscale image.
    """
    # --- LOGGING SETUP ---
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_filename = f"log_bilateral_params_raw_{timestamp}.txt"
    original_stdout = sys.stdout
    log_file = open(log_filename, 'w')
    sys.stdout = log_file

    print(f"--- Starting Bilateral Filter Parameter Optimization (on Raw Grayscale) ---")
    ...
    try:
        ...
        # --- ArUco Detector Initialization (with optimal params) ---
        ...
        aruco_params.cornerRefinementWinSize = 30
        aruco_params.cornerRefinementMaxIterations = 70
        aruco_params.cornerRefinementMinAccuracy = 0.001
        aruco_detector = cv2.aruco.ArucoDetector(aruco_dict, aruco_params)

        # --- PARAMETER SEARCH RANGES ---
        d_range = [5, 9, 13]
        sigma_color_range = [25, 50, 75, 100]
        sigma_space_range = [25, 50, 75, 100]
        ...
        param_combinations = list(itertools.product(d_range, sigma_color_range, sigma_space_range))
        print(f"Starting parameter search. Testing {len(param_combinations)} combinations.")
        print("IMPORTANT: For best results, keep the camera and ArUco marker STATIONARY during the test.")

        for i, (d, sigma_color, sigma_space) in enumerate(param_combinations):
            print(f"\n--- Testing combination {i+1}/{len(param_combinations)}: d={d}, sigmaColor={sigma_color}, sigmaSpace={sigma_space} ---")
            ...
            while frames_collected < FRAMES_PER_TEST:
                try:
                    ...
                    filtered_frame = cv2.bilateralFilter(gray_frame, d, sigma_color, sigma_space)

                    aruco_corners_cropped, aruco_ids, _ = aruco_detector.detectMarkers(filtered_frame)
                    ...
                    print("\nSocket timeout. No data received. Make sure the stream is active.")
                    break
            ...
            if len(poses) > 10:
                ...
                print(f"\nJitter Score (std_dev sum): {jitter_score:.6f} (x: {std_dev[0]:.4f}, y: {std_dev[1]:.4f}, yaw: {std_dev[2]:.4f})")

                if jitter_score < lowest_jitter_score:
                    lowest_jitter_score = jitter_score
                    best_params = {'d': d, 'sigmaColor': sigma_color, 'sigmaSpace': sigma_space}
                    print(f"*** New best parameters found! ***")
            else:
                print("\nNot enough data collected to calculate jitter score for this combination.")
        ...
        if best_params:
            ...
            print("Could not determine optimal parameters. Please check the data stream and settings.")
        print("===================================================")
