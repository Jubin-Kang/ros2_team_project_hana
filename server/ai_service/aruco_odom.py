...
class ArucoReceiverNode(Node):
    def __init__(self):
        ...
        self.odom_pub = self.create_publisher(Odometry, '/DP_08/odom_aruco', qos_profile_sensor_data)
    ...
    def receive_and_process(self):
        try:
            ...
            aruco_corners_cropped, aruco_ids, _ = self.aruco_detector.detectMarkers(binary_frame)

            display_frame = transformed_frame.copy()

            if aruco_ids is not None:
                # 5. Estimate 3D pose for position
                crop_x_min, crop_y_min = data['offsets']
                aruco_corners_full_frame = [c + np.array([crop_x_min, crop_y_min], dtype=np.float32) for c in aruco_corners_cropped]
                ...
                corners = aruco_corners_cropped[0].reshape((4, 2))
                direction_vec = corners[1] - corners[0]
                ...
                # 8. Visualization
                transformed_corners = [cv2.perspectiveTransform(c.reshape(-1, 1, 2), self.M) for c in aruco_corners_cropped]
                ...
                for i in range(len(rvecs)):
                    ...
                    cv2.line(display_frame, tuple(pts[0]), tuple(pts[1]), (0,0,255), 2)
                    cv2.line(display_frame, tuple(pts[0]), tuple(pts[2]), (0,255,0), 2)
                    cv2.line(display_frame, tuple(pts[0]), tuple(pts[3]), (255,0,0), 2)
                ...
                # If no markers detected, republish the last known pose
